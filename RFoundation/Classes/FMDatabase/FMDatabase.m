...
/**
 *
 * the two methods are added for iPhone convinient db access 
 * the db name in sandbox and bundle should be same,  
 * before use, you may need define DB_NAME first, DB_NAME can be the db name in bundle 
 * Then you can acess db with FMDatabase *db = [FMDatabase database];
 *  
 */
#define DB_NAME @"fmdb.db"

+ (id)database {
	return [FMDatabase databaseWithDBName:DB_NAME];
}

+ (id)databaseWithDBName:(NSString *)sandboxDBName {
	...
	NSString *realDBPath = [documentFolder stringByAppendingPathComponent:sandboxDBName];
		
	if (![fileManager fileExistsAtPath:realDBPath]) {
		NSString *dbBundlePath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:sandboxDBName];
		NSError *error = nil;
